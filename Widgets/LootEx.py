...
def Initialize_And_Load():
    ...
    ui.module_window.window_size = settings.current.window_size
    ui.module_window.window_pos = settings.current.window_position
...
def main():
    ...
    if not UIManager.IsWorldMapShowing() and not GLOBAL_CACHE.Map.IsMapLoading() and not GLOBAL_CACHE.Map.IsInCinematic() and not Player.InCharacterSelectScreen():
