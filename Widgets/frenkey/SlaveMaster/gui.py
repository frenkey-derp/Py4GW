    ...
    def draw_command_bar(self):
        ...
        if UIManagerExtensions.IsElementVisible(self.frame_id):
            ...
            if PyImGui.is_rect_visible(1, 1):
                ...
                self.options.Following = ImGui.toggle_button(IconsFontAwesome5.ICON_RUNNING + "##Following", self.options.Following,button_size,button_size)
                ImGui.show_tooltip("Following")
                PyImGui.table_next_column()
                self.options.Avoidance = ImGui.toggle_button(IconsFontAwesome5.ICON_PODCAST + "##Avoidance", self.options.Avoidance,button_size,button_size)
                ImGui.show_tooltip("Avoidance")
                PyImGui.table_next_column()
                self.options.Looting = ImGui.toggle_button(IconsFontAwesome5.ICON_COINS + "##Looting", self.options.Looting,button_size,button_size)
                ImGui.show_tooltip("Looting")
                PyImGui.table_next_column()
                self.options.Targeting = ImGui.toggle_button(IconsFontAwesome5.ICON_BULLSEYE + "##Targeting", self.options.Targeting,button_size,button_size)
                ImGui.show_tooltip("Targeting")
                PyImGui.table_next_column()
                self.options.Combat = ImGui.toggle_button(IconsFontAwesome5.ICON_SKULL_CROSSBONES + "##Combat", self.options.Combat,button_size,button_size)
                ...
                for command in self.commands_list:
                    PyImGui.table_next_column()
                    if ImGui.button(command.icon + f"##{command.name}", button_size, button_size):
                        command.execute()
