from LootEx.ItemAction import InventoryAction, ItemActions, ItemAction
...
def CreateItemActions() -> list[InventoryAction]:
    ...
    for bag_id in bag_ids:
        ...
        for item in bag_items:
            ...
            item_action.SourceBag = bag_id
            item_action.SourceSlot = item.slot
            item_actions.append(item_action)
            
    # Filter out the _None actions
    item_actions = [action for action in item_actions if action.Action != ItemAction._None]
...
def GetItemAction(item_id: int) -> ItemAction:
    ...
    profile = Settings.Current.LootProfile
    ...
    if profile.Filters:
        for filter in profile.Filters:
            if filter.HandlesItemId(item_id):
                action = filter.GetAction(instance_type)
                return action
...
class ItemMoveAction:
    ...
    def execute(self):            
        ...
        Inventory.MoveItem(self.item_id, self.target_bag, self.target_slot, self.quantity)
...
def CompactInventory() -> bool:
    ...
    item_typeOrder += [int(item) for item in ItemType if int(item) not in item_typeOrder]
    ...
    def GetBagFromSlot(slot: int) -> tuple[Bags, int]:
        backpack_size = PyInventory.Bag(Bags.Backpack.value, Bags.Backpack.name).GetSize()
        belt_pouch_size = PyInventory.Bag(Bags.BeltPouch.value, Bags.BeltPouch.name).GetSize()
    ...
    for bag_id in bag_ids:
        ...
        for item in bag_items:
            target_bag, target_slot =  GetBagFromSlot(item_array.index(item.item_id))
            item_action_array.append(ItemMoveAction(item.item_id, bag_id, target_bag, item.slot, target_slot))
...
def HasItemToIdentify() -> tuple[bool, int]:    
    for bag_id in range(Bags.Backpack, Bags.Bag2 + 1):
        ...
        for item_id in item_array:
            ...
            ConsoleLog("LootEx", "Item to identify: " + Item.GetName(item_id) + " (Id: "+ str(item_id), Console.MessageType.Info)
            return True, item_id
...
def IdentifyItem(item_id) -> bool: 
    ...
    if item_id != -1:
        ConsoleLog("LootEx", "Identify item: " + Item.GetName(item_id), Console.MessageType.Info)
...
def HasItemToSalvage() -> tuple[bool, int]:    
    for bag_id in range(Bags.Backpack, Bags.Bag2 + 1):
        ...
        for item_id in item_array:
            ...
            ConsoleLog("LootEx", "Item to salvage: " + Item.GetName(item_id) + " (Id: "+ str(item_id) + ")", Console.MessageType.Info)
            return True, item_id
    return False, -1
...
def SalvageItem(item_id) -> bool:
    ...
    if item_id != -1:
        ConsoleLog("LootEx", "Salvage item: "+ str(item_id) + " with salvage kit "  + str(salvage_kit) , Console.MessageType.Info)
...
def SetupItemsToBuy() -> bool:    
    ...
    idtificationKits = Inventory.GetModelCount(ModelID.Superior_Identification_Kit)
    if(idtificationKits < Settings.Current.LootProfile.IdentificationKits):            
        idtificationKits_to_buy = Settings.Current.LootProfile.IdentificationKits - idtificationKits
    ...
    if(salvageKits < Settings.Current.LootProfile.SalvageKits):            
        salvageKits_to_buy = Settings.Current.LootProfile.SalvageKits - salvageKits
    ...
    if(expertSalvageKits < Settings.Current.LootProfile.ExpertSalvageKits):            
        expertSalvageKits_to_buy = Settings.Current.LootProfile.ExpertSalvageKits - expertSalvageKits
        if expertSalvageKits_to_buy > 0:
            merchant_item_list = Trading.Merchant.GetOfferedItems()
            merchant_item_list = ItemArray.Filter.ByCondition(merchant_item_list, lambda item_id: Item.GetModelID(item_id) == ModelID.Expert_Salvage_Kit)
    ...
    if(lockpicks < Settings.Current.LootProfile.Lockpicks):            
        lockpicks_to_buy = Settings.Current.LootProfile.Lockpicks - lockpicks
        if lockpicks_to_buy > 0:
            merchant_item_list = Trading.Merchant.GetOfferedItems()
            merchant_item_list = ItemArray.Filter.ByCondition(merchant_item_list, lambda item_id: Item.GetModelID(item_id) == ModelID.Lockpick)
