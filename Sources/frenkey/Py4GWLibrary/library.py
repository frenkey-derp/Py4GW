...
from Sources.frenkey.Py4GWLibrary.enum import SortMode, LayoutMode, ViewMode
from Sources.frenkey.Py4GWLibrary.module_cards import draw_compact_widget_card, draw_compact_widget_card, draw_widget_card
...
class ModuleBrowser:
            ...
            case LayoutMode.Library:
                match self.view_mode:
                    case ViewMode.Favorites:
                        prefiltered = [w for w in self.widgets if w in self.favorites]
                ...
                self.filtered_widgets = [w for w in prefiltered if 
                                        ...
                                        (not filter_text or filter_text.lower() in w.plain_name.lower() or filter_text.lower() in w.folder.lower())]
    ...
    def draw_global_toggles(self, width : float, item_spacing : float = 5):
        buttons = 4
        button_width = (width - item_spacing * (buttons - 1)) / buttons
    ...
    def compact_view(self):
        ...
        if ImGui.Begin(ini_key=self.ini_key, name=self.module_name, flags=PyImGui.WindowFlags.NoResize):   
            win_size = PyImGui.get_window_size()
            self.win_size = (win_size[0], win_size[1])
            style = ImGui.get_style()
            
            self.draw_global_toggles(PyImGui.get_content_region_max()[0], style.ItemSpacing.value1)
            ...
            if self.filtered_widgets and self.widget_filter:
                ...
                height = min(200, len(self.filtered_widgets) * 28 + (style.WindowPadding.value2 or 0) * 2)
                ...
                if PyImGui.begin(
                    ...
                    if suggestion_hovered and not self.context_menu_id:
    ...
    def draw_libary(self):
        PyImGui.set_next_window_size(self.win_size, PyImGui.ImGuiCond.Always)
        if ImGui.Begin(ini_key=self.ini_key, name=self.module_name):    
            io = PyImGui.get_io()
            win_size = PyImGui.get_window_size()
            self.win_size = (win_size[0], win_size[1])
            ...
            if ImGui.begin_table("navigation_view", 2, PyImGui.TableFlags.SizingStretchProp):
