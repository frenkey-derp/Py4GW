...
BONUS_TAG = "<c=@ItemBonus>"
DULL_TAG = "<c=@ItemDull>"

BONUS_RE = re.compile(rf'{BONUS_TAG}(.*?)</c>')
DULL_RE = re.compile(rf'{DULL_TAG}(.*?)</c>')


def extract_item_bonus_blocks(text: str) -> str:
    lines = text.splitlines()
    result = []

    i = 0
    while i < len(lines):
        bonus_match = BONUS_RE.search(lines[i])
        ...
        if bonus_match:
            ...
            dull_text = None
            if i + 1 < len(lines):
                dull_match = DULL_RE.search(lines[i + 1])
                if dull_match:
                    dull_text = dull_match.group(1)

            if dull_text:
                result.append(f"{bonus} {dull_text}")
                i += 2
                continue
            else:
                result.append(bonus)
...
def align_bonus_lines(text: str) -> str:
    lines = text.splitlines()
    result = []
    ...
    while i < len(lines):
        ...
        if BONUS_TAG in line and i + 1 < len(lines) and DULL_TAG in lines[i + 1]:
...
class Upgrade:
    ...
    def description_short(self) -> str:
        if self.encoded_description:
            ...
            if decoded_description:
                return extract_item_bonus_blocks(decoded_description)
    ...
    def description(self) -> str:
        if self.encoded_description:
            ...
            if decoded_description:
                return align_bonus_lines(decoded_description)
