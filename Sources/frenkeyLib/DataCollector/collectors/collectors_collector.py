...
class CollectorsCollector(ListCollector[Collector]):
    def __init__(self, get_local_path, get_default_path, *, version = '1.0', value_type = None, key_decoder = None, key_encoder = None):
        ...
        self.map_collectors : set[Collector] = set()
        self.unrevealed_map_collectors : set[Collector] = set()
        ...
        if self.unrevealed_map_collectors:            
            ...
            for agent_id in agent_ids:
                ...
                if updated_collector:
                    self.unrevealed_map_collectors.discard(updated_collector)
    ...
    def _flush_cache(self):
        ...
        map_collectors = [collector for collector in self if collector.map_id == self.current_map_id and collector.HasMissingData()]
        self.map_collectors.update(map_collectors)
        self.unrevealed_map_collectors.update([collector for collector in map_collectors if collector.position == (0.0, 0.0) or collector.map_id == 0 or collector.model_id == 0])
